...
report.get('/:id(\\d+)', function (req, res) {
    if (req.params.id) {
        console.log('\nGET ' + req.originalUrl);
        var report = cache.get('grbi-' + req.params.id);
        if (report !== null && req.app.get('cacheManager') === true) {
            res.send(report);
        } else {
            var report = {
                'report_id': '',
...
report.get('/list', function (req, res) {
    ...
    var reports = cache.get('garl-' + JSON.stringify(req.query));
    if (reports !== null && req.app.get('cacheManager') === true) {
        res.send(reports);
    } else {
        var sql = null;
            ...
            if (!err) {
                ...
                connectionPool.query(sql, function (err, data) {
                    if (!err) {
                        console.log('DB Result: ' + JSON.stringify(data));
                        if (req.app.get('cacheManager') === true) {
                            cache.put('garl-' + JSON.stringify(req.query), data, 36000);
                        }
                        res.json(200, data);
                    } else {
                        console.log('DB ERROR: ' + err.message);
                    ...
                    connectionPool.query(sql, function (err, data) {
                        if (!err) {
                            ...
                            var sql = squel.insert().into('report_item')
                                .set('report_id', req.body.report_id)
